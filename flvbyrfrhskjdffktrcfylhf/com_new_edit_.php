...
if (isset($_POST['sub_comm_new_edit']) and $_POST['sub_comm_new_edit'] == 'Изменить') 
{
...
else {$_SESSION['mcomm_new_edit'] = 3; header('Location: com_new_edit.php?id='.$_POST['id'].'&post='.$_POST['post'].''); exit();}

	//Создаем запрос для записи данных в БД
	$result = mysql_query ("UPDATE comments SET author='".$author."', text='".$text."' WHERE id='".$id."'",$db);
		
	if (!$result) {$_SESSION['mcomm_new_edit'] = 2; header('Location: com_new_edit.php?id='.$id.'&post='.$post.''); exit();}
	unset($_POST['author']); unset($_POST['text']); unset($_POST['id']); 
	$_SESSION['mcomm_new_edit'] = 4; header('Location: com_new_edit.php?id='.$id.'&post='.$post.''); exit();
